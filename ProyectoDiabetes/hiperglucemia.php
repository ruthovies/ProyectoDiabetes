...
if (isset($_POST['accion']) && $_POST['accion'] == 'cargar_edicion') {
    $stmt = $pdo->prepare("SELECT * FROM hiper WHERE idUsuario = ? AND fecha = ? AND tipoComida = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if ($accion == 'agregar') {
            ...
            $stmt->execute([$idUsuario, $fecha, $tipoComida, $hora, $glucosa, $correccion]);
        } elseif ($accion == 'editar') {
            $sql = "UPDATE hiper SET hora = ?, glucosa = ?, correccion = ? WHERE idUsuario = ? AND fecha = ? AND tipoComida = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$hora, $glucosa, $correccion, $idUsuario, $fecha, $tipoComida]);
...
$registros = $pdo->query("SELECT * FROM hiper WHERE idUsuario = {$_SESSION['user_id']} ORDER BY fecha DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
    ...
    <title>Gestión de Hiperglucemia</title>
...
<body>
    ...
    <div class="container text-center">
        <h2 class="mb-3">Gestión de Hiperglucemia</h2>
        ...
        <form method="POST" class="mb-4">
            ...
            <div class="mb-3">
                <label class="form-label">Corrección de Insulina</label>
                <input type="number" name="correccion" class="form-control" required value="<?= $editData['correccion'] ?? '' ?>">
            </div>
            
            <button type="submit" class="btn btn-primary btn-custom"><?= $editData ? 'Actualizar Registro' : 'Agregar Registro' ?></button>
        </form>

        <h3 class="mt-3">Registros de Hiperglucemia</h3>
        <table class="table table-striped table-bordered">
            <thead class="table-dark">
                <tr>
                    ...
                    <th>Hora</th>
                    <th>Glucosa (mg/dL)</th>
                    <th>Corrección Insulina (unidades)</th>
                    <th>Acciones</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= htmlspecialchars($registro['correccion']) ?></td>
                    <td>
