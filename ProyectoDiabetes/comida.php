...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        } elseif ($accion == 'editar' && $tipoComida) {
            $sql = "UPDATE comidas SET fecha = ?, pre = ?, post = ?, racion = ?, insulina = ?, tipoComida = ? WHERE tipoComida = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$fecha, $pre, $post, $racion, $insulina, $tipoComida, $tipoComida]);
        } elseif ($accion == 'eliminar' && $tipoComida) {
            $sql = "DELETE FROM comidas WHERE tipoComida = ?";
            $stmt = $pdo->prepare($sql);
...
<body>
    ...
    <div class="container text-center">
        ...
        <form method="post" class="mb-4">
            ...
            <input type="hidden" name="tipoComida" value="<?= $editComida['tipoComida'] ?? '' ?>">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="tipoComida" class="form-control" placeholder="Desayuno, Almuerzo, Cena..." required value="<?= $editComida['tipoComida'] ?? '' ?>">
            </div>
